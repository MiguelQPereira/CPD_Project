...
typedef struct particle_{
    ...
    int n_local; // number of local part
    int true_neighbor; // 1 if all neighbors in the local
...
int simulation(double space_size, long grid_size, long long num_particles, long long num_timesteps, particle_t *par){
    ...
    int n_total_cells = pow((space_size*grid_size),2);
    cell* cells = malloc(n_total_cells * sizeof(cell));

    int cells_per_process = n_total_cells / size;
    int remainder = n_total_cells % size;

    int *local_cells_counts = malloc(size * sizeof(int));
    int *displacements = malloc(size * sizeof(int));

    for (int i = 0; i < size; i++) {
        local_cells_counts[i] = cells_per_process;  
        
        if (i < remainder) 
            local_cells_counts[i]++;
        
        if (i == 0) {
            displacements[0] = 0;  // O primeiro processo começa no início do array
        } else {
            // Cada processo começa onde o anterior terminou
            displacements[i] = displacements[i-1] + local_cells_counts[i-1];
        }
    }

    int local_num_cells = local_cells_counts[rank];
    cell *local_cells = malloc(local_num_cells * sizeof(cell));

    MPI_Scatterv(
        cells,                           // Buffer de origem (todas as celulas, só no rank 0)
        local_cells_counts,         // Quantas partículas cada processo recebe
        displacements,                 // Deslocamentos no array global
        MPI_BYTE,                      // Tipo de dado (usamos BYTE para struct)
        local_cells,               // Buffer de destino (partículas locais)
        local_num_cells * sizeof(cell),  // Quantidade a receber
        MPI_BYTE,                      // Tipo de dado
        0,                             // Rank root (quem envia)
    ...
    for(int i = 0; i < num_timesteps; i++){
        ...
        int local_grid_size = grid_size / size;
        int start_x = rank * local_grid_size;
        int end_x = (rank + 1) * local_grid_size;
    
        // Handle remainder if grid_size not divisible by size
        if (rank == size - 1) {
            end_x = grid_size;
        }
