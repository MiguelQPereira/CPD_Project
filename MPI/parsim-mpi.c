        ...
        for (int id_par=0; id_par < st_par[cell].n_particles; id_par++){
            ...
            if (new_cell != cell){
                ...
                if(rank == 0 && new_cell >= work_size[0] + work_size[1]){
                    ...
                    if(new_cell + start_point < aux || new_cell >= aux + work_size[psize-1])
                        printf("--t=%d Rank:%d sending to rank %d particle in cell: %d, particle id: %d\n", t, rank, prev_rank, new_cell, st_par[cell].par[id_par].id);
                    ...
                    //if (/*new_cell + start_point < aux )||*/ new_cell >= work_size[rank])
                    ...
                    //if (/*new_cell+start_point < aux )||*/ new_cell >= work_size)
                        //printf("--t=%d Rank:%d sending to rank %d particle in cell: %d\n", t, rank, next_rank, new_cell);
                ...
                if (id_par != st_par[cell].n_particles-1){
                    ...
                    if (id_par < -1) {
                        fprintf(stderr, "Erro: id_par < -1 após ajuste!\n");
                        exit(1);
                    }
                }
        ...
        for(int j=0; j< work_size[rank] ;j++){
            ...
            for(int k = 0; k<st_par[j].n_particles; k++){  
                ...
                if(px->alive == 1){
                    ...
                    printf("Fx: %.14f \n Fy: %.14f alive: %d\n\n", (int) (px->x/(space_size/grid_size)), (int) (px->y/(space_size/grid_size)), px->alive);
                }
            }
        }
